...
const renderFilter = (name, count) => {
  ...
  div.addEventListener('click', onFilterClick);
  return div;
};

const renderCard = (color, type, description) => {
  const div = document.createElement(`div`);
  div.innerHTML = `<article class="card card--${color} card--${type}">
...
const renderAllFilters = names => {
  ...
  names.forEach(name =>
    fragment.appendChild(renderFilter(name, getRandomNumber(1, 20))),
  );
  return fragment;
};
